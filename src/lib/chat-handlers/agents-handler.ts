  ...
  async handleChat(
    ...
    try {
      ...
      if (mcpServerBaseUrl.includes("localhost") || mcpServerBaseUrl.includes("127.0.0.1")) {
        throw new Error("MCP server URL must be publicly accessible (no localhost URLs)");
      }

      // Create hosted MCP tool using the official hosted MCP approach
      debug.log("AGENTS-HANDLER", `Creating hosted MCP tool for: ${mcpServerUrl}`);
      ...
      debug.log("AGENTS-HANDLER", `Agent created successfully with model: ${modelName}`);
      ...
      const userInput = typeof lastMessage?.content === 'string' 
        ...
        ? lastMessage.content.map(part => 
            ...
            'type' in part && part.type === 'text' ? part.text : 
            '[non-text content]'
          ).join(' ')
        : "";
      ...
      debug.log("AGENTS-HANDLER", `Running agent with native streaming, input: ${userInput.substring(0, 100)}...`);
      
      const streamResult = await withTrace(`${this.displayName} AI Assistant`, async () => {
        ...
        switch (event.type) {
          case 'raw_model_stream_event':
            ...
            if (event.data.type === 'model' && 
              ...
              debug.log("AGENTS-HANDLER", `Model event: ${event.data.event.type}`);
            }
            
            // Handle text streaming - SDK confirmed to NOT auto-stream to our controller
            if (event.data.type === 'model' && event.data.event.type === 'response.output_text.delta') {
              ...
              if (delta && sequenceNumber) {
                ...
                if (this.processedSequenceNumbers.has(sequenceNumber)) {
                  debug.log("AGENTS-HANDLER", `⏩ DUPLICATE sequence ${sequenceNumber} - skipping`);
                  return;
                }
          ...
          case 'run_item_stream_event':
            ...
            debug.log("AGENTS-HANDLER", `Item stream: ${event.name} - ${typeof event.item === 'object' && event.item !== null && 'type' in event.item ? (event.item as { type: string }).type : 'unknown'}`);
            
            if (event.name === 'tool_called') {
              // Show tool call in real-time as it happens
              let toolName = 'unknown_tool';
              
              // Extract tool name generically
              if (typeof event.item === 'object' && event.item !== null && 'rawItem' in event.item) {
                const rawItem = (event.item as { rawItem: Record<string, unknown> }).rawItem;
                if ('name' in rawItem) {
                  toolName = String(rawItem.name);
                  
                  // For MCP calls, try to get the specific function name
                  if (rawItem.name === 'mcp_call' && 'providerData' in rawItem &&
                      typeof rawItem.providerData === 'object' && rawItem.providerData !== null &&
                      'name' in rawItem.providerData) {
                    const providerData = rawItem.providerData as Record<string, unknown>;
            ...
            if (event.name === 'tool_output') {
              // Show tool completion in real-time
              let toolName = 'unknown_tool';
          ...
          default:
            debug.log("AGENTS-HANDLER", `Unhandled stream event: ${(event as { type: string }).type}`);
            break;
        }
      }
